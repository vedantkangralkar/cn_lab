...
def topology():
    ...
    print("*** Starting ping to h1 in background")
    ...
    print("*** Disconnecting from AP2 and measuring handover delay")

    start_time = time()
    sta1.cmd('iw dev sta1-wlan0 disconnect')
    sleep(5)  # give some time for disconnection

    print("*** Connecting to AP1")
    sta1.cmd('iw dev sta1-wlan0 connect ap1-ssid')

    # Wait for connection confirmation (poll every second, timeout after 10 sec)
    connected = False
    for i in range(10):
        sleep(1)
        output = sta1.cmd('iw dev sta1-wlan0 link')
    ...
    if connected:
        print(f"*** Handover completed in {end_time - start_time:.2f} seconds")
        print(sta1.cmd('iw dev sta1-wlan0 link'))
    else:
        print("*** Handover failed: sta1 did not connect to AP1 within timeout")
